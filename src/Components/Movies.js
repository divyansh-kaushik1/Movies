...
export default class Movies extends Component {
    ...
    loadMoreMovies = async() => {
        ...
        const res = await axios.get(`https://api.themoviedb.org/3/movie/popular?api_key=4c7f3a58ebfd9cc7e7d89e366994307a&language=en-US&page=${newPage}`);
        let data = res.data
        // console.log(data);
        this.setState({
    ...
    async componentDidMount(){
        //Side effects 
        const res = await axios.get(`https://api.themoviedb.org/3/movie/popular?api_key=4c7f3a58ebfd9cc7e7d89e366994307a&language=en-US&page=${this.state.currPage}`);
        let data = res.data
        // console.log(data);
        this.setState({
            movies:[...data.results]
                        ...
                        <div className="movies-list">
                            {
                                this.state.movies.map((movieObj)=>(
                                    <div className="card movies-card" onMouseEnter={()=>this.setState({hover:movieObj.id})} onMouseLeave={()=>this.setState({hover:''})}>
                                        <img src={`https://image.tmdb.org/t/p/original${movieObj.poster_path}`}  alt={movieObj.title} className="card-img-top movies-img"/>
